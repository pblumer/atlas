...
- **The hosted example apps find the instance they just started again, on an engine of
  ...
  What the scoping does not repair is that finding your own instance by elimination is
  still a guess when two callers start the same definition at once. Closing that means
  the start answering with the key it minted, which is a change to the engine's command
  path rather than to a page, and is proposed in
  [ADR-draft-starting-an-instance-returns-its-key](docs/adr/draft-starting-an-instance-returns-its-key.md).
