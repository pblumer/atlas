...
| [`pruefe-datensaetze.bpmn`](pruefe-datensaetze.bpmn) + [`pruefe-datensaetze.dmn`](pruefe-datensaetze.dmn) + [`row-correction-form.json`](row-correction-form.json) | CSV batch validation **with a per-row correction loop** (ADR-0084 + ADR-0085): upload a CSV via `POST /api/v1/processes/{key}/instances-from-csv`, then a **multi-instance subprocess** runs a **business rule task** per row against the DMN decision `RowValid` (email/group/licence). A valid row collects its `verdict`; an invalid one parks on a **correction user task** in the Tasks app, and once corrected it re-validates (loop until valid). Completes when every row is valid; the DMN worker evaluates in-process. |
