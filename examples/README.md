...
> **Reading them as a user, not as a developer?** The handbook's **Beispiele**
> chapter (`/handbuch.html#beispiele`) describes every example here in both
> German and English — what it is for and how it is built — renders its diagram,
> and installs it into your own instance in one click. It also carries the
> per-worker commissioning runbooks (`/handbuch.html#runbooks`): what to do at
> Google, Microsoft or Atlassian before a task can run. This file stays the
> developer-facing reference: the verified runs, the element visits, the MCP calls.
>
> The two do not drift apart. `go test ./examples` regenerates the catalogue the
> chapter serves (`api/web/examples-catalog.json`) from the files here and fails
> when an example has no card, when a card names an example that does not exist,
> or when two examples would ship the same form id.
...
| [`entra-verzeichnis-abgleich.bpmn`](entra-verzeichnis-abgleich.bpmn) | **Verzeichnis-Abgleich (Entra ID)** (ADR-0332): das Delta-Beispiel zu Ende gedacht — hier *führt* Atlas seine eigenen Konten und Gruppen aus dem Mandanten nach. Timer-Start `R/PT1H` (ADR-0051), dann `GET /api/v1/directory-sync` für die beiden Cursor und die Revision, `delta-users` und `delta-groups` ab diesen Cursorn, und `POST /api/v1/directory-sync` meldet beides zurück. **Der erste Lauf schreibt nichts**: der Meldetask trägt `apply: false`, entscheidet aber alles und legt einen Bericht ab — dieselbe Auswertung, nur der letzte Schritt fällt aus, und der Cursor bleibt stehen, solange nichts geschrieben wurde. Der Schalter heisst `apply` und nicht `dryRun`, weil ein fehlendes JSON-Feld der Nullwert ist und der „nichts schreiben“ heissen muss. Braucht einen Entra-Worker (lesend: `User.Read.All`, `Group.Read.All`) und einen REST-Worker „atlas“ mit einem API-Token des Scopes `directory` — der erreicht genau diese beiden Routen und kann insbesondere nicht deployen. |
| [`bestandsaufnahme.bpmn`](bestandsaufnahme.bpmn) | **Bestandsaufnahme (Entra ID)** ([ADR-0333](../docs/adr/0333-inventory-commissioning-load.md)): was am ersten Tag schon da war, als vorbestehend eintragen. `delta-groups` als vollständige Aufzählung, dann `POST /api/v1/inventory-load` — die Rechte bekommen den Ursprung `legacy`, weil Atlas sie nicht vergeben hat und das auch sagen soll. **Ohne Timer**: eine Bestandsaufnahme wird angestossen, nicht eingeplant, und der Trockenlauf (`apply: false`) ist nur dann ein Schutz, wenn jemand den Bericht liest. Sie trägt ausschliesslich ein — ein Recht, das die Meldung nicht nennt, wird nie entzogen, weil eine Meldung die Teilantwort eines Systems ist und Schweigen kein Beleg. Braucht einen Entra-Worker (lesend) und einen REST-Worker „atlas“ mit einem API-Token des Scopes `inventory`. |
| [`abgleich.bpmn`](abgleich.bpmn) | **Abgleich (Entra ID)** ([ADR-0334](../docs/adr/0334-reconciliation.md)): täglich fragen, ob noch stimmt, was Atlas über die Zugriffsrechte behauptet. Timer `R/P1D`, `delta-groups`, dann `POST /api/v1/reconciliation`. **Hier bedeutet Schweigen etwas** — anders als bei der Bestandsaufnahme: ein geführtes Recht, das die Lesung nicht nennt, gilt als fehlend. Deshalb nennt der Lauf in `refs`, was er **vollständig** gelesen hat, und ausserhalb davon wird nichts geschlossen. Der Prozess verändert nichts; übernehmen, entziehen oder streichen sind drei einzelne Handlungen eines Menschen unter **Operations → Reconciliation**. Gleiche Voraussetzungen wie die Bestandsaufnahme. |
| [`austrittspruefung.bpmn`](austrittspruefung.bpmn) | **Austrittsprüfung (Entra ID)** ([ADR-0334](../docs/adr/0334-reconciliation.md)): derselbe Abgleich um die andere Achse — nicht „stimmt diese Gruppe noch“, sondern „hält diese Person noch irgendwo etwas“. Der Lauf nennt in `subjects` die Person, deren Bestand er vollständig gelesen hat; aus Gruppenlesungen liesse sich diese Frage nicht zusammensetzen. **Das leere Ergebnis ist die Antwort**: der Bericht sagt im Klartext, dass die Person draussen ist. Ein Subjekt ohne Konto gilt dabei nie als sauber — die Abwesenheit eines Kontos ist nicht die Abwesenheit von Zugriff. Start mit der Variable `person` (Entra-Objekt-Id), ohne Timer. |
| [`rezertifizierung.bpmn`](rezertifizierung.bpmn) | **Rezertifizierung (Entra ID)** (ADR-draft-access-recertification): quartalsweise fragen, ob die Rechte, die Leute haben, noch gebraucht werden. `delta-users` samt `manager`, dann `POST /api/v1/recertification` — daraus wird je gehaltenem Recht eine Frage, zugeordnet an die Führungskraft der haltenden Person. **Wer prüft, bestimmt das Modell und nicht Atlas**: eine Vorgesetztensuche ist eine Verzeichnisfrage, genau wie bei der Genehmigungsregel `superior`. Wen die Abbildung nicht nennt, dessen Zeile ist nicht zugeordnet und landet beim Eröffnenden — kein Fehler, sondern die Antwort „niemand“. **Schweigen ist nie eine Entscheidung**: unbeantwortete Zeilen bleiben unbeantwortet, auch wenn die Kampagne schliesst. Beantwortet wird einzeln unter **Tasks → Access review**; mehrere auf einmal ist bewusst nicht vorgesehen. |
| [`befristung.bpmn`](befristung.bpmn) | **Befristung** (ADR-draft-time-bounded-entitlements): täglich zurückgeben, was hätte enden sollen. Ein Produkt nennt mit `maxDays` eine Obergrenze; wer darunter bestellt, bekommt ein Recht mit Enddatum. **Ablauf ist nicht Entzug** — am Tag danach hat das Zielsystem die Mitgliedschaft noch, also bleibt das Recht geführt und wird als überfällig gemeldet. Zurückgegeben wird über die **Bestellzeile**: nur ein bestelltes Recht trägt je ein Ende, und deren Rückgabe entzieht nach der eingefrorenen Freigabe — stärker als der Weg, den Abgleich und Rezertifizierung nehmen müssen. Die Obergrenze gilt nie für ein gefundenes Recht: dessen Startdatum ist der Moment des Findens. Braucht einen REST-Worker „atlas“ mit einem Token der Rolle `user`. |
| [`erinnerung/`](erinnerung/) | **Erinnerung** (ADR-draft-pending-work): wöchentlich jeder prüfenden Person sagen, was auf sie wartet. `GET /api/v1/recertification`, dann je Person eine **Aufruf-Aktivität** als Multi-Instance über `distinct values(...)` — eine Person mit vierzig Zeilen bekommt eine Mail und nicht vierzig. Der gerufene Prozess fragt `GET /api/v1/pending-work?principal=…` und schickt nur bei `counts.total > 0`. **Atlas verschickt nichts**; das Modell tut es, mit dem Mail-Task, den es längst gibt. Der Parameter ist dem **Operator** vorbehalten — ein Portal, in dem jeder die offenen Posten jedes anderen aufzählen kann, ist ein Organigramm mit Arbeitslasten daran. Braucht einen REST-Worker „atlas“ mit einem Token des Scopes `reminders` an einem Konto mit der Rolle `operator`, und einen Mail-Worker „portal“. |
| [`unvereinbarkeit.bpmn`](unvereinbarkeit.bpmn) | **Unvereinbarkeit** (ADR-draft-conflicting-rights): wöchentlich melden, wer eine Kombination hält, die der Katalog verbietet. Erklärt wird sie als dritte Kantenart `excludes` — die **einzige symmetrische**, weshalb die Freigabe beide Richtungen aufschreibt: wer nur eine läse, fände die Hälfte der Verletzungen und meldete den Bestand als halb sauber. Eine Bestellung, die eine verbotene Kombination herstellen würde, wird **abgelehnt**, gegen Gehaltenes und gegen denselben Warenkorb. **Rückwirkend gilt die Regel trotzdem** — anders als eine Befristung, die Teil des Vergebenen ist. **Ein Konflikt hat keinen Schuldigen**: nichts entzieht automatisch, der Befund nennt beide Seiten, und gehandelt wird über Rückgabe oder Rezertifizierung. Braucht einen REST-Worker „atlas“ (Rolle `operator`) und einen Mail-Worker „portal“. |
...
| [`account-bestellung/`](account-bestellung/) | **Account-Bestellung (Entra ID)** ([ADR-0128](../docs/adr/0128-process-applications.md) + ADR-0029 + ADR-0172): eine Applikation mit **öffentlichem Startformular**, DMN-gesteuertem Kontotyp-Mapping (`KontotypMapping`: Kürzel, `accountEnabled`, Kategorie, `usageLocation`), menschlicher Freigabe und Entra-Provisionierung — plus ein einbettbares **HTML+JS-Widget**, das die Bestellung von einer beliebigen Website aus startet. Der Lehrsatz ist das **fail-closed Gatter**: ein Exklusiv-Gateway prüft, ob der UPN dem Testmuster entspricht, und der *Default* ist das Ende **ohne** jeden Entra-Aufruf — ein Gateway, dessen Bedingung wegen eines Tippfehlers nicht greift, darf kein Konto anlegen. Schreibt scharf gegen **blumer.net**, aber nur gegen klar benannte Test-Objekte. |
| [`entra-onboarding-selfservice/`](entra-onboarding-selfservice/) | **Onboarding-Self-Service (Entra ID)** (ADR-0172): der Fachbereich bestellt den Arbeitsplatz selbst, die IT gibt frei. Startformular → Script-Task (`displayName`/`mailNickname` vorschlagen) → dasselbe fail-closed Testgatter wie oben → User-Task „freigeben" (der Admin setzt das Initialpasswort, es wird **nie** mitbestellt) → `create-user` → zwei **optionale** Schritte (Lizenz, Gruppe), die je an einem Gateway auf „ist das Feld gesetzt" hängen und auf dasselbe Ende zulaufen — zweimal dasselbe Muster statt eines Zweigwaldes. |
| [`benutzerverwaltung/`](benutzerverwaltung/) | **Atlas verwaltet seine eigenen Benutzer** ([ADR-0122](../docs/adr/0122-protected-system-project-and-bootstrap-deployment.md) + [ADR-0123](../docs/adr/0123-sanctioned-user-provisioning-for-system-processes.md)): Aufnahme, Zugriffs-Review und Offboarding als drei Prozesse mit sechs Formularen — der erste Insasse des **geschützten System-Projekts**. Die Abgrenzung ist der Punkt: die native Users-Konsole bleibt der *direkte, synchrone* Weg, diese Prozesse sind die *Vordertür für regierte Änderungen* (Antrag, Freigabe, Audit, Mail), und beide schreiben in **denselben** User-Store. Der Schreibpfad ist ein enger `userConnector`, der **nur** für Prozesse des System-Projekts läuft. Kopien liegen unter [`api/systemprocesses/`](../api/systemprocesses/), sind ins Binary eingebettet und werden beim Serverstart idempotent deployed — bei Änderungen **beide Stellen** angleichen. |
| [`identitaet-lebenszyklus/`](identitaet-lebenszyklus/) | **Eine Prozessinstanz je Identität** (ADR-0028 + ADR-0040): kann *eine* Instanz je Mensch den kompletten Lebenszyklus führen, inklusive aller bestellten Business Services mit ihrem Zustand? Ja — mit zwei dauerhaft nebenläufigen Token im Wurzel-Scope: **Spur A** ist der Lebenszyklus (jeder Zustand ein Receive-Task, der Token steht *in* dem Kästchen, das den Zustand benennt), **Spur B** ein ereignisbasiertes Gateway in einer Endlosschleife als Ereignis-Hub. Warum *nicht* ein Ereignis-Subprozess je Bestellung: der schreibt still nichts (Daten-Assoziationen werden nur im Wurzel-Scope verdrahtet), und ein nicht unterbrechender Boundary-Event feuert genau einmal. Die gemessene Grenze liegt **nicht** im Schreibpfad, sondern im Lesepfad — Details in [TESTBERICHT.md](identitaet-lebenszyklus/TESTBERICHT.md). Daneben ein Mengentest-Seeder, der zugleich der Aufräum-Hebel ist. |
| [`ad-objektmodell/`](ad-objektmodell/) | **ArchiMate-Export nach Atlas** ([ADR-0230](../docs/adr/0230-process-information-model.md) + ADR-0232): kein Prozess, sondern eine Untersuchung — warum ein sauber gepflegtes ArchiMate-3.1-Modell **nicht** als Informationsmodell importierbar ist, und wie der Export am Ende doch in Atlas landete. Atlas liest genau zwei Dokumente (eigenes JSON, UML XMI 2.5.1); die Formaterkennung wählt bei ArchiMate den XMI-Pfad und findet nichts, weil dessen Elemente unter `<elements>` stehen. Der Unterschied ist aber nicht nur syntaktisch: ArchiMate kennt für ein `BusinessObject` **keine typisierten Attribute** — und genau die braucht ein Informationsmodell, damit der `itemSubjectRef` eines Datenobjekts etwas zum Auflösen hat. Mit beiden Konvertierungsskripten und beiden Zwischenständen. |
| [`mahnwesen/`](mahnwesen/) | **Offene Rechnung mahnen** — das Beispiel zur **Eskalation**, der einzigen Ereignisart, die einen Fall aus einem Teilprozess nach oben meldet, *ohne ihn abzubrechen*. Am Teilprozess „Zahlung überwachen" (Timer → Mahnung → Timer → Eskalation) hängen zwei Boundaries, und ihr Unterschied ist die ganze Fachlichkeit: die **Nachricht** `zahlung.eingegangen` **unterbricht** (der Mahnlauf ist gegenstandslos), die **Eskalation** unterbricht **nicht** (`cancelActivity="false"` — der Lauf endet ordentlich *und* der Inhaber wird gefragt). Der Teilprozess ist keine Kosmetik: eine Eskalation wird an der umschliessenden Aktivität gefangen, ohne sie gäbe es keine. Die Fristen sind Startvariablen, weil `<timeDuration>` FEEL nimmt (ADR-0055) — mit `PT2S` läuft derselbe Prozess in Sekunden statt Wochen. Der Mahn-Task ist ein Mockup, das Modell läuft also ohne Mailserver. |
| [`preisaenderung/`](preisaenderung/) | **Preisänderung erreicht jede offene Offerte** — das Beispiel zum **Signal**, und die Gegenprobe zu `mahnwesen/`: eine *Nachricht* trifft genau eine Instanz (die mit passendem Korrelationsschlüssel), ein *Signal* trifft **alle**, die gerade warten, und weiss nicht, wie viele das sind — weshalb es bewusst keinen Schlüssel hat. `preisliste.bpmn` wirft, `offerte.bpmn` fängt an einer **nicht unterbrechenden** Boundary; unterbrechend würde jede Preisänderung jede offene Offerte vernichten. Das Ergebnis landet in `angebotNeu` und überschreibt `angebot` nicht: was dem Kunden genannt wurde, ist ein Fakt und keine Variable. Verifiziert: drei wartende Offerten, **eine** Instanz des Werfers, drei nachgerechnete Angebote. |
| [`reisestorno/`](reisestorno/) | **Reise buchen und sauber rückabwickeln** — das Beispiel zur **Kompensation**: ein *Fehler* springt aus einer Aktivität heraus, die gerade schiefging, eine *Kompensation* macht Aktivitäten rückgängig, die längst **erfolgreich** abgeschlossen sind. Flug und Hotel tragen je eine Kompensations-Boundary, an der über eine `<association>` ein Task mit `isForCompensation="true"` hängt — **kein** Sequenzfluss, sonst liefe er immer. Zwei Wurf-Ereignisse wickeln **rückwärts** ab (erst Hotel, dann Flug), weil Stornofristen sich unterscheiden. Der Beweis steht in den Variablen: mit `zahlungOk: false` stehen `hotelstorno` und `flugstorno` auf der Instanz, mit `true` fehlen beide. Alle Umsysteme sind Mockups. |
| [`umzug/`](umzug/) | **Umzug organisieren** — Atlas für eine *Privatperson*, und das einzige Modell im Repo mit einem **Termin** statt einer Dauer: `<timeDate>` nimmt einen FEEL-Ausdruck (ADR-0055), der mit echter Datumsarithmetik rechnet (Umzugstermin − Kündigungsfrist). Drei Stränge laufen parallel (kündigen, Umzugsfirma, Adressen), ein **paralleles** Join wartet auf alle drei — ein exklusives wäre der häufigste Modellierfehler überhaupt. Zwei Stolpersteine stecken in dem einen Timer-Ausdruck, beide beim Bauen tatsächlich passiert und beide mit demselben Incident (`result is not a valid date`): das `date(...)` um die Variable (eine Prozessvariable wird als JSON persistiert und kommt als String zurück) und ein **Offset statt einer Zonen-ID** (`"09:00:00Z"` geht, `"09:00:00@Europe/Zurich"` nicht). |
