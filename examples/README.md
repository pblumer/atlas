...
| [`rezertifizierung.bpmn`](rezertifizierung.bpmn) | **Rezertifizierung (Entra ID)** (ADR-0341): quartalsweise fragen, ob die Rechte, die Leute haben, noch gebraucht werden. `delta-users` samt `manager`, dann `POST /api/v1/recertification` — daraus wird je gehaltenem Recht eine Frage, zugeordnet an die Führungskraft der haltenden Person. **Wer prüft, bestimmt das Modell und nicht Atlas**: eine Vorgesetztensuche ist eine Verzeichnisfrage, genau wie bei der Genehmigungsregel `superior`. Wen die Abbildung nicht nennt, dessen Zeile ist nicht zugeordnet und landet beim Eröffnenden — kein Fehler, sondern die Antwort „niemand“. **Schweigen ist nie eine Entscheidung**: unbeantwortete Zeilen bleiben unbeantwortet, auch wenn die Kampagne schliesst. Beantwortet wird einzeln unter **Tasks → Access review**; mehrere auf einmal ist bewusst nicht vorgesehen. |
