...
| [`pruefe-datensaetze.bpmn`](pruefe-datensaetze.bpmn) + [`pruefe-datensaetze.dmn`](pruefe-datensaetze.dmn) | CSV batch validation (ADR-0084): upload a CSV via `POST /api/v1/processes/{key}/instances-from-csv`, then a **multi-instance subprocess** runs a **business rule task** per row against the DMN decision `RowValid` (email/group/licence), collecting one structured `verdict` per row into `verdicts`. Self-completing; the DMN worker evaluates in-process. |
