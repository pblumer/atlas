...
4. **LDAP hardening.** Paged results, per-value modify, an mTLS bind and connection
   pooling are **done** (ADR-0154, amended 2026-08-21). The delta read remains — and
   it is not one feature but two vendor protocols, DirSync for AD and RFC 4533
   elsewhere, so the AD half belongs with the AD connector rather than with a generic
   one guessing which server it is talking to. Delta is what makes a directory
   connector usable for reconciliation, so it is the most valuable item left in this
   wave.
