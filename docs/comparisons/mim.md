...
| MIM connector | Atlas | Status |
|---|---|---|
| Microsoft Graph Connector | `entra` (ADR-0172) | **Partial** — the directory surface: create / read / list / update / delete a user, enable, disable, and group membership. A listing follows Graph's paging itself, so a model never sees `@odata.nextLink`. Other Graph areas (licences, directory roles, administrative units) and advanced queries (`endsWith`, `$search`) go through the `rest` connector. |
