...
| *(no MIM counterpart — the cloud directory)* | `entra` (ADR-draft-entra-id-connector) | **Implemented** — the same lifecycle against Entra ID over Graph. |
...
| MIM connector | Atlas | Status |
|---|---|---|
| Microsoft SQL Server | `mssql` (ADR-draft-generic-sql-connector) | **Implemented** — query / query-one / execute, bound parameters, named binding. |
| Generic SQL Connector | `mssql`, `mariadb`, `postgres` (ADR-draft-generic-sql-connector) | **Partial** — three products rather than one generic kind, deliberately: placeholder syntax is per-product, so the product is part of the model. |
...
| MIM connector | Atlas | Status |
|---|---|---|
| Microsoft Graph Connector | `entra` (ADR-draft-entra-id-connector) | **Partial** — the directory surface: create / read / update / delete a user, enable, disable, and group membership. Other Graph areas (licences, directory roles, administrative units) go through the `rest` connector. |
| Microsoft Azure Active Directory Connector | `entra` (ADR-draft-entra-id-connector) | Out of support in MIM itself; superseded by Graph, which is what this connector speaks. |
...
| LDAP Data Interchange Format (LDIF) | `ldif` (ADR-draft-directory-file-connector) | **Implemented** — read and write. Change records are read as entries, not applied. |
...
1. ~~**Generic SQL connector.**~~ **Done** ([ADR-draft-generic-sql-connector](../adr/draft-generic-sql-connector.md))
...
2. ~~**Microsoft Graph / Entra ID connector.**~~ **Done**
   ([ADR-draft-entra-id-connector](../adr/draft-entra-id-connector.md)) as `entra`, worker-only, with the
...
5. ~~**File connector family.**~~ **Done.** Fixed-width, AVP and a write direction
   ...
   `ldif` connector (ADR-draft-directory-file-connector), because they carry directory entries rather than table
