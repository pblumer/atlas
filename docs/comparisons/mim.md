...
| MIM connector | Atlas | Status |
|---|---|---|
| Microsoft Graph Connector | `entra` (ADR-0172) | **Partial** — the directory surface: create / read / list / update / delete a user, enable, disable, reset password, group membership, create and delete a group, and stand up a Team on a group with members. A listing follows Graph's paging itself, so a model never sees `@odata.nextLink`, and can opt into Graph's advanced query support (`ConsistencyLevel: eventual` plus `$count=true`) for `endsWith`, `ne`, `not` and `$search`. Other Graph areas (licences, directory roles, administrative units) and `$orderby` go through the `rest` connector. |
...
5. ~~**File connector family.**~~ **Done.** Fixed-width, AVP and a write direction
   ...
   `ldif` connector (ADR-0171), because they carry directory entries rather than table
