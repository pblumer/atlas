...
- **Open question:** How often two callers start the same definition inside one batch is
  ...
  if it never happens, the recipe clients use today is already correct and this record is
  work for nothing. Nothing in the engine counts it, and the demo apps that would hit it
...
repaired that, and a guard over the embedded pages now refuses the unscoped read.
...
- **Positive:** a client stops guessing. The four recipes under `api/web` collapse into
  reading one field, the concurrent-start collision disappears rather than being made
  rarer, and the guard test over the embedded pages stops being the only thing standing
  between a client and a capped listing. Anything written against the endpoint in future
  gets the answer without having to know how the instance list is ordered.
