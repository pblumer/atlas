...
| `ATLAS_AD_MOCK`, `ATLAS_AD_MOCK_SEED` | Read by `atlas worker --connector ad`: serve Active Directory tasks against a mock directory in the worker's memory, optionally seeded from an LDIF or DSML file. The models are unchanged and nothing reaches a domain controller ([ADR-draft-ad-connector-mock-mode](adr/draft-ad-connector-mock-mode.md)) |
