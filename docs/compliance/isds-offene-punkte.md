...
| O-02 | **Feingranulare Autorisierung** | P1 | R-04 | Für **Maschinen** ist es gelöst: ein API-Token trägt einen fail-closed begrenzten Geltungsbereich und ist nie admin (ADR-0194). Für **Personen** wird weiterhin nur `admin` erzwungen; Projekt-Sichtbarkeit mit `viewer`/`editor` (ADR-0071) und Gruppen (ADR-0180) existieren, und Konnektoren sind seit M11 erfasst — Eigentümer, Freigabe an Personen und Gruppen, Anspruch des Abonnements auf seinen Nachrichtennamen ([`ADR-0205`](../adr/0205-connector-ownership-and-event-delivery.md)). Was bleibt, ist die Fläche selbst: **nachgemessen 2026-08-28 sind von 199 `/api/v1`-Routen 53 auf `admin` geprüft und 146 für jedes angemeldete Konto erreichbar** — darunter `POST /api/v1/deployments` (Codeausführung, R-09), `POST /api/v1/scripts/run`, Instanzabbruch und die Variablen jeder Instanz. Ein API-Token trägt ausserdem **gar keine Rolle** | Rollen pro Endpunktgruppe, deklariert je Route statt in 52 Handler-Aufrufen: Konzept M9 und [`ADR-draft-roles-per-endpoint-group`](../adr/draft-roles-per-endpoint-group.md). Darüber hinaus offen: Sichtbarkeit von Prozessvariablen an eine Berechtigung binden |
