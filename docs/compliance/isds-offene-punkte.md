...
|-----|-------|------|--------|-------------|-----------|
| O-01 | **Föderierte Authentisierung** (OIDC/SAML, Ziel eIAM) | P1 | R-03 | Lokale Passwörter (bcrypt, Session-Cookie) **und**, wo eine Betreiberin einen Anbieter konfiguriert, OpenID Connect. `Source`/`ExternalID` tragen jetzt die Verknüpfung zum Subjekt des Anbieters; die Naht ist weiterhin eine einzige — eine föderierte Anmeldung endet in derselben Session wie eine lokale. Die vier Rollen aus [ADR-0209](../adr/0209-roles-per-endpoint-group.md), auf die Claims abgebildet werden, existieren seit M9 | **Schritt 1 ist gebaut** (M12, [`ADR-0210`](../adr/0210-federated-authentication.md)): Atlas ist OIDC-Relying-Party — Anmeldefluss mit PKCE, Prüfung des ID-Tokens gegen die JWKS des Anbieters, Kontoanlage beim ersten Login mit der Rolle `user`, Schaltfläche auf der Anmeldemaske. Ohne konfigurierten Anbieter ändert sich nichts. Offen: die Abbildung von Claims auf Rollen und Gruppen (Schritt 2), die vom Anbieter ausgelöste Abmeldung, SAML falls eIAM es verlangt, und die Doku für den Betrieb hinter einem authentisierenden Proxy |
