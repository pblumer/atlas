...
| O-07 | ~~**Authentisierung für `/mcp` und `/metrics`**~~ | — | R-08 | **Erledigt.** `/mcp` ist transportseitig durch `--auth` geschützt und ein Werkzeugaufruf handelt mit dem Credential des Aufrufers (ADR-draft-authenticated-mcp-transport); `/metrics` verlangt ein API-Token mit Geltungsbereich `metrics` (ADR-draft-metrics-behind-the-boundary). Welche Route offen ist, ist deklariert und per Test gegen eine ausgeschriebene Liste gehalten (ADR-draft-route-access-classes) | — |
