...
| O-07 | **Authentisierung für `/mcp` und `/metrics`** | P2 | R-08 | `/mcp` ist für externe Aufrufer transportseitig unauthentisiert (ADR-0016); intern authentisiert der Adapter mit einem Service-Token (ADR-0049). `/metrics` ist nicht durch `--auth` geschützt | Optionale Authentisierung bzw. eigener Listener/Bind-Adresse für beide, damit die Absicherung nicht allein an der Proxy-Konfiguration hängt |
