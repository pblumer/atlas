...
|-----|-------|------|--------|-------------|-----------|
| O-01 | **Föderierte Authentisierung** (OIDC/SAML, Ziel eIAM) | P1 | R-03 | Nur lokale Passwörter (bcrypt, Session-Cookie). Die Haken `Source`/`ExternalID` sind da und werden von nichts gelesen; die Authentisierungsgrenze ist bewusst austauschbar gebaut (ADR-0044), und sie hat **genau eine Naht**: `handleLogin` ist die einzige Stelle, an der eine Session entsteht. Die vier Rollen aus [ADR-0209](../adr/0209-roles-per-endpoint-group.md), auf die Claims abgebildet werden, existieren seit M9 | **Ausgearbeitet als Massnahme M12**: [`ADR-draft-federated-authentication`](../adr/draft-federated-authentication.md) und Kapitel M12 im [Zugriffsschutz-Konzept](zugriffsschutz-konzept.md). Zu bauen: der OIDC-Relying-Party-Fluss samt Token-Prüfung, danach die Abbildung von Claims auf Rollen und Gruppen, dazu die Doku für den Betrieb hinter einem authentisierenden Proxy |
