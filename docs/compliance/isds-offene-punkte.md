...
| O-02 | **Feingranulare Autorisierung** | P1 | R-04 | Erzwungen wird nur `admin`; Projekt-Sichtbarkeit mit `viewer`/`editor` (ADR-0071) und Gruppen (ADR-0180) existieren. `POST /api/v1/deployments` ist für jeden angemeldeten Benutzer offen | Rollen pro Endpunktgruppe (deployen, starten, Laufzeitdaten lesen, Aufgaben bearbeiten, administrieren); Trennung «Modellierer» / «Betrieb» / «Sachbearbeiter»; Sichtbarkeit von Prozessvariablen an eine Berechtigung binden |
| O-03 | **Sicherheits-Audit-Log** | P1 | R-13 | Fachliche Spuren sind stark (Ereignis-Log, Variablen-Audit ADR-0098, Task-Audit ADR-0159). An-/Abmeldungen, Fehlversuche und Administrationsaktionen werden **nicht** protokolliert; es gibt keinen HTTP-Access-Log | Eigene, stabil benannte Log-Ereignisse für Login/Logout/Fehlversuch, Benutzer- und Rollenänderung, Secret- und Connector-Änderung, Backup/Restore, Deploy — inklusive Akteur; maschinenlesbar (`--log-format=json`) und an ein SIEM lieferbar |
| O-04 | **Anmelde-Härtung** | P1 | R-03, R-12 | Mindestlänge 8 Zeichen; keine Sperre und keine Verzögerung nach Fehlversuchen; der Token-Bucket schützt nur die öffentlichen Routen (`api/ratelimit.go`) | Rate-Limit und/oder temporäre Sperre auf `/api/v1/auth/login`; konfigurierbare Passwortregeln; optionale MFA (oder ausdrücklicher Verweis auf O-01 als Ersatz) |
...
| O-07 | **Authentisierung für `/metrics`** | P2 | R-08 | **`/mcp` ist erledigt**: transportseitig durch `--auth` geschützt, und ein Werkzeugaufruf handelt mit dem Credential des Aufrufers (ADR-draft-authenticated-mcp-transport). Welche Route offen ist, ist deklariert und per Test belegt (ADR-draft-route-access-classes). `/metrics` ist weiterhin nicht durch `--auth` geschützt | Eigener Listener bzw. Bind-Adresse (`--metrics-addr`) oder eine Zugriffsklasse `operator` für `/metrics`, damit seine Absicherung nicht allein an der Proxy-Konfiguration hängt |
