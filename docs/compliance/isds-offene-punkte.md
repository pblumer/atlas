...
|-----|-------|------|--------|-------------|-----------|
| O-01 | **Föderierte Authentisierung** (OIDC/SAML, Ziel eIAM) | P1 | R-03 | Lokale Passwörter (bcrypt, Session-Cookie) **und**, wo eine Betreiberin einen Anbieter konfiguriert, OpenID Connect. `Source`/`ExternalID` tragen die Verknüpfung zum Subjekt des Anbieters; die Naht ist weiterhin eine einzige — eine föderierte Anmeldung endet in derselben Session wie eine lokale. Die vier Rollen aus [ADR-0209](../adr/0209-roles-per-endpoint-group.md), auf die Claims abgebildet werden, existieren seit M9 | **Beide Schritte sind gebaut** (M12, [`ADR-0210`](../adr/0210-federated-authentication.md)): Atlas ist OIDC-Relying-Party — Anmeldefluss mit PKCE, Prüfung des ID-Tokens gegen die JWKS des Anbieters, Kontoanlage beim ersten Login, Schaltfläche auf der Anmeldemaske — und eine Administratorin kann einen Claim auf Rollen und Gruppen abbilden (`/api/v1/settings/oidc-mapping`, Console → Organization → Single sign-on). Die Abbildung ist aus, bis jemand sie einschaltet; ein Claim vergibt nur durch Treffer, nie durch Abwesenheit, und `user` bleibt ein Boden. Ohne konfigurierten Anbieter ändert sich nichts. Offen: die vom Anbieter ausgelöste Abmeldung, SAML falls eIAM es verlangt, und die Doku für den Betrieb hinter einem authentisierenden Proxy |
| O-02 | **Feingranulare Autorisierung** | P1 | R-04 | **Weitgehend erledigt** (M9, [`ADR-0209`](../adr/0209-roles-per-endpoint-group.md)): Jede der 199 `/api/v1`-Routen nennt die Rolle, die sie verlangt — `admin` (51), `modeler`, `operator`, `user` —, und die Schranke setzt sie an genau einer Stelle durch, für Session, API-Token, Deploy-Token und OAuth-Freigabe gleichermassen; ein Inventar-Test hält die administrative Menge gegen eine ausgeschriebene Liste. Für **Maschinen** war es mit ADR-0194 gelöst (fail-closed begrenzter Geltungsbereich, nie admin); ein Token trägt jetzt zusätzlich die Rollen seiner Ausstellerin. Dazu: Projekt-Sichtbarkeit mit `viewer`/`editor` (ADR-0071), Gruppen (ADR-0180) und Worker mit Eigentümer, Freigabe und Anspruch auf den Nachrichtennamen ([`ADR-0205`](../adr/0205-connector-ownership-and-event-delivery.md)) | Beim Update behält jedes bestehende Konto `modeler`+`operator`+`user`; die Rollen bewusst zu vergeben bleibt eine Betriebsaufgabe, die keine Änderung am Code ist. **Die Sichtbarkeit von Prozessvariablen ist inzwischen an eine Berechtigung gebunden** ([`ADR-0275`](../adr/0275-instance-visibility.md)): `GET /api/v1/instances/{key}/variables` antwortet ganz für Operator, Administratorin und Mitglieder des Projekts, aus dem die Definition deployt wurde, für Inhaberinnen einer Task auf der Instanz nur mit den Feldern, nach denen deren Formular fragt, und sonst mit 404 — auch über MCP, das dieselbe Route mit dem Credential der Aufruferin ruft. Offen im Code: `GET /api/v1/tasks/{key}` hat dieselbe Lücke in kleinerem Format, und `GET /api/v1/tasks` gibt ungefiltert alle offenen User Tasks der Installation heraus — beides Metadaten, keine Variablenwerte ([#1011](https://github.com/pblumer/atlas/issues/1011)); die übrigen instanzbezogenen Lesezugriffe sind über die Rolle statt über die Beziehung geschützt, darunter `/timeline`, `/data-objects` und `/variable-audit`, die dieselben vertraulichen Werte tragen wie `/variables` ([#1012](https://github.com/pblumer/atlas/issues/1012)); und ein feinerer Schnitt (etwa `deployer`) fehlt, sobald jemand danach fragt |
| O-03 | **Sicherheits-Audit-Log** | P1 | R-13 | **Weitgehend erledigt** (ADR-0197): stabile `auth.*`-Ereignisse für An-/Abmeldung, Fehlversuch mit Grund, Drosselung, Autorisierungsverweigerung, Konto-Lebenszyklus, Passwortsetzung und Deploy-Token — je Zeile mit Akteur und Absenderadresse, ohne Secret, maschinenlesbar mit `--log-format=json` | Noch nicht abgedeckt: Secret- und Worker-Änderungen, Backup/Restore und Deploy als eigene Ereignisse; ein HTTP-Access-Log im Produkt (bleibt Proxy-Aufgabe, M-06) |
...
2. ~~**O-02**~~ ✅ dann **O-01** — die Rollen sind mit M9 definiert (`admin`,
   `modeler`, `operator`, `user`), und eine externe Identität bekommt genau sie
   zugewiesen; umgekehrt wäre ein Mapping ins Leere entstanden.
