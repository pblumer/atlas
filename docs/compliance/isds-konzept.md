...
| R-04 | **Grobgranulare Autorisierung**: erzwungen wird nur die Rolle `admin` plus Projekt-Sichtbarkeit; jeder angemeldete Benutzer darf deployen, Instanzen starten, Laufzeitdaten inkl. Prozessvariablen lesen — und jeden Konnektor auflisten, ändern, löschen sowie darauf ein Inbound-Abonnement anlegen | gelb |
...
| R-04 | **Grobgranulare Autorisierung.** Nur `admin` erzwungen; jeder angemeldete Benutzer darf deployen, starten, abbrechen und Prozessvariablen aller Instanzen lesen. Konnektoren stehen ganz ausserhalb des Modells: kein Eigentümer, freie Änderung und Löschung durch jedes Konto, und ein Inbound-Ereignis wird allein über seinen Nachrichtennamen zugestellt (Zugriffsschutz-Konzept 1.5, Massnahme M11) | Vertraulichkeit, Integrität | M-05, M-07, enger Benutzerkreis, getrennte Installationen | **gelb** |
