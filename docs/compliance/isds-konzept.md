...
| R-04 | **Grobgranulare Autorisierung**: erzwungen wird nur die Rolle `admin` plus Projekt- und Konnektor-Sichtbarkeit; von 199 `/api/v1`-Routen sind 146 für jedes angemeldete Konto erreichbar — darunter deployen (Codeausführung), Skripte ausführen, Instanzen abbrechen und die Variablen jeder Instanz lesen (Massnahme M9) | gelb |
...
| R-04 | **Grobgranulare Autorisierung.** Nur `admin` erzwungen: von 199 `/api/v1`-Routen sind 146 für jedes angemeldete Konto erreichbar — deployen (Codeausführung), Skripte ausführen, Instanzen abbrechen, Prozessvariablen aller Instanzen lesen. Ein API-Token trägt gar keine Rolle. Konnektoren sind seit M11 erfasst (Eigentümer, Freigabe, Anspruch auf den Nachrichtennamen); die Fläche selbst ist Massnahme **M9** | Vertraulichkeit, Integrität | M-05, M-07, enger Benutzerkreis, getrennte Installationen | **gelb** |
