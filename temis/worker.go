...
// (ADR-0050). Register it with a [job.Runner] via HandleWithOutput for the reserved
...
// client. A job whose connector is not registered leaves the job pending with an
// error, exactly like any worker failure. sink, if non-nil, observes each result.
func Handler(store *state.Store, lookup dmn.ProcessLookup, reg *Registry, sink func(dmn.Result)) job.OutputHandler {
