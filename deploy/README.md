...
to start strict mode when the host or container runtime cannot enforce it, and
likewise when an enabled interpreter cannot start inside it — each one is run once
at boot rather than taken on trust.
