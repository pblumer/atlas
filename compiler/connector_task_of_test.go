...
// a persisted connector job outlives the process definition that compiled its
// element as a connector task. Resolving it must return an error (so the worker
...
func TestConnectorTaskOfStaleJob(t *testing.T) {
	// Element 0 claims a connector-task detail index, but the connector-task table
	...
	// A node that is not a connector task at all (Detail == -1, the "none" sentinel).
	p.nodes = []CompiledNode{{Detail: -1}}
	if _, err := p.ConnectorTaskOf(0); err == nil {
		t.Fatal("expected error for a non-connector-task node, got nil")
	}

	// Happy path: a valid connector task returns its detail without error.
	...
	if got.Connector != 7 {
		t.Fatalf("got connector %d, want 7", got.Connector)
	}
}
