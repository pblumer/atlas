...
// the transaction's cancel boundary (ADR-0105). It carries no detail (a cancel always
...
// error boundary and always interrupting (ADR-0105). Returns its element id.
...
// SetTransaction marks an already-added subprocess node as a <transaction> (ADR-0105), so the
