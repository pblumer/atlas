...
func registerScope(
	...
	for _, ev := range c.IntermediateCatchEvents {
		...
		case ev.Link != nil:
			// A link catch is the landing point of a link throw of the same name (ADR-0126).
	...
	for _, ev := range c.IntermediateThrowEvents {
		...
		if ev.Link != nil {
			// A link throw is a goto to the link catch of the same name (ADR-0126). It compiles
...
func connectScope(b *Builder, ids map[string]int32, c *xmlFlowContent) error {
	...
	// Resolve link events within this scope (ADR-0126): a link throw is a goto to the link
