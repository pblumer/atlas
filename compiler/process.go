...
const (
	...
	TypeScriptJobTask     // a script task authored in a general-purpose language (PowerShell, …) that runs via the job path, not inline like a FEEL script task (ADR-0044); like a service task it creates a job and waits
...
// task delegates to the DMN worker (ADR-0044). Like a service task it runs as a
