...
// connector is down". These drive the traversal directly through a harness, so each
...
//    │                   │
//    │                   └──uses──> restricted (a connector outside our access)
...
const graph = {
  ...
  edges: [
    ...
    { from: "process:2", to: "connector:c1", kind: "uses" },
...
// diagram: everything that would notice if this connector stopped answering.
test("dependents answer what breaks if a connector goes down", async ({ page }) => {
  const r = await impact(page, "connector:c1", { direction: "dependents", depth: Infinity });

  expect(new Set(r.nodes)).toEqual(new Set(["connector:c1", "process:2", "process:1"]));
...
test("dependencies answer what a process needs", async ({ page }) => {
  ...
  expect(new Set(r.nodes)).toEqual(
    new Set(["process:1", "process:2", "connector:c1", "restricted:1"]),
  );
});
...
test("depth bounds the walk", async ({ page }) => {
  const one = await impact(page, "connector:c1", { direction: "dependents", depth: 1 });
  expect(new Set(one.nodes)).toEqual(new Set(["connector:c1", "process:2"]));

  const two = await impact(page, "connector:c1", { direction: "dependents", depth: 2 });
  expect(new Set(two.nodes)).toEqual(new Set(["connector:c1", "process:2", "process:1"]));
});
