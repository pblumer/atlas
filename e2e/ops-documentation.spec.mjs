...
test("an executed element shows what the modeler documented about it", async ({ page }) => {
  ...
  // The author's paragraph break survives into the panel.
  await expect(doc.locator("p")).toContainText("Bei fehlenden Unterlagen ablehnen.");
  const white = await doc.locator("p").evaluate((el) => getComputedStyle(el).whiteSpace);
