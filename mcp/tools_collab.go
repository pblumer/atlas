...
// collabTools expose ADR-0140's live modeling sessions to an AI agent, so Claude
// can join a draft and co-edit it alongside people in real time (the ADR-0140 M2
	...
	return []Tool{
		{
			...
			Description: "Join a draft's live collaboration session to co-edit it with people in real time " +
				"(ADR-0140). Returns a sync snapshot: your own participant id (\"self\"), the current " +
		...
		},
		{
			...
			Description: "Read what changed in a draft's live session since your last poll: the buffered " +
				"presence / lock / change events plus the current participants and locks (ADR-0140). This is " +
		...
		},
		{
			...
			Description: "Acquire or release a per-element edit lock in a draft's live session (ADR-0140). " +
		...
		},
		{
			...
			Description: "Broadcast an element edit to a draft's live session so every open canvas updates " +
				"immediately (ADR-0140). Lock the element with atlas_session_lock first. This relays the " +
		...
		},
		{
			...
			Description: "Announce which element you are focused on in a draft's live session, so people see " +
				"where you are working (ADR-0140). Optional but collaborative.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"draftId":       draftIDProp,
		...
		},
		{
			...
			Description: "Leave a draft's live session, releasing any element locks you held (ADR-0140). " +
