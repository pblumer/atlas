...
const WORKER_TYPES = [
  ...
  },
  {
    ...
    desc: "The model an \u003cb\u003eagent-driven ad-hoc subprocess\u003c/b\u003e asks which of its tools to run next. What an agent may reach is the diagram: the contained activities no sequence flow leads to are its tools, named by their element ids, described by the modeller\u0027s own documentation. A round is one job and a tool call one activity, so the loop is durable and replayable \u2014 and it never runs in the engine, because one model call can take minutes and hang. Two wire formats: Messages (Anthropic, and OpenRouter\u0027s Messages-compatible endpoint) and Chat Completions (OpenAI, and anything OpenAI-compatible). Configure each model below: its API key lives in the vault and never enters a model, and the model named here is the \u003cb\u003edefault\u003c/b\u003e \u2014 a task or a container may name its own, so one Worker serves a cheap classification and a strong piece of advice. The same configuration also serves the \u003cb\u003eAI Task\u003c/b\u003e: a service task that asks a model once and puts the answer in a variable, with no tools, because a step with tools is the container. Worker-only, and Atlas supervises the worker for it.",
