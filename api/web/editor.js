...
// stKindRow renders one picker row. The badge marks the kinds Atlas does *not* run itself
// — the other way round from before, when seventeen of nineteen rows carried an
// "in-engine" badge. A mark almost everything has distinguishes nothing; it only costs
// reading time. What that badge said is not lost: inEngineNoticeHTML still says it in
// full under the kind actually chosen, which is where it bears on a decision (ADR-0164).
  ...
  return `
    <div class="stkind-row" data-kind="${k.id}" data-match="${esc((k.name + " " + k.desc).toLowerCase())}"
      ...
      <span style="line-height:1.25"><b>${esc(k.name)}</b>${runsInEngine(k) ? ""
        : `<span class="stkind-onworker" title="Atlas does not run this itself — an external worker serves it.">on a worker</span>`}<br>
