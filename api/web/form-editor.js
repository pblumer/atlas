...
export async function mountFormEditor(root, { api, toast, formId, projectId }) {
  ...
  (async () => {
    const workers = await generationWorkers(api);
    if (gen !== generation || !workers.length) return;
    genBtn.hidden = false;
    genBtn.addEventListener("click", async () => {
      commit(activeTab); // generate over what is on screen, not over the last commit
      // The form is passed in so a second generation refines it rather than
      // replacing it, and the id so the result comes back under the form's own.
      const got = await openFormGenerator({ api, workers, formId: currentId() || id, schema });
