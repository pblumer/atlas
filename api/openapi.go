...
func (s *Server) apiRoutes() []apiRoute {
	return []apiRoute{
		...
		{"POST", "/api/v1/restore", s.handleRestore, apiOp{
			summary: "Restore design-time data from an uploaded backup archive; overwrites matching artifacts, skips anything outside the design-time allowlist, and needs a restart for deployed processes to take effect. A deployed definition whose key already names a different definition here is NOT taken, and neither is the archive's node identity — a key and an identity belong to the installation that issued them (admin-only when auth is on) (ADR-0107/ADR-draft-a-portable-backup-does-not-overwrite-another-installations-identity)", tag: "System", role: RoleAdmin,
