...
// --- Part 2: workers and decisions as dependency nodes ---

// TestDeriveGraphDrawsWorkerDependencies: a model names a worker by name only and
// cannot tell whether that name is configured anywhere (ADR-0036/0041). The mesh is
// on the outside, where the configured workers are, so it can answer — which is the
// whole reason the references are enumerable (ADR-0158).
func TestDeriveGraphDrawsWorkerDependencies(t *testing.T) {
	p := proc(1, "invoice", "Invoice", "a1")
	p.Workers = []WorkerUse{{ElementID: "Task_1", Name: "billing-rest", TargetID: "c1"}}

	g := DeriveGraph(Landscape{
		Applications: []Application{app("a1", "Billing")},
		Processes:    []Process{p},
		Workers:      []Worker{{ID: "c1", Name: "billing-rest", Type: "rest", CanView: true}},
	}, Options{})

	if n := nodeByID(t, g, "worker:c1"); n.Kind != KindWorker || n.Name != "billing-rest" {
		t.Errorf("worker node = %+v", n)
	}
	if !hasEdge(g, "process:1", "worker:c1", EdgeUses) {
		t.Errorf("worker edge missing from %#v", g.Edges)
	}
}

// TestWorkerNodeCarriesNoEndpointOrCredential is the disclosure bound. A worker
// record holds an endpoint and a credential reference; neither belongs in a
// landscape picture that anyone with modeler access can open, and an internal
...
func TestWorkerNodeCarriesNoEndpointOrCredential(t *testing.T) {
	p := proc(1, "invoice", "Invoice", "a1")
	p.Workers = []WorkerUse{{ElementID: "Task_1", Name: "billing-rest", TargetID: "c1"}}
	...
	g := DeriveGraph(Landscape{
		...
		Processes:    []Process{p},
		Workers: []Worker{{
			ID: "c1", Name: "billing-rest", Type: "rest", CanView: true,
	...
	for _, leak := range []string{"internal-billing", "corp.example", "vault://billing", "CredentialsRef", "endpoint"} {
		if strings.Contains(string(body), leak) {
			t.Errorf("worker node leaks %q in %s", leak, body)
		}
	}
}

// TestDeriveGraphMarksAnUnconfiguredWorkerUnresolved is the operational answer
// the model cannot give: this process asks for a worker nobody configured, so it
// would fail at run time. The name is safe to show — it is in the caller's own model.
func TestDeriveGraphMarksAnUnconfiguredWorkerUnresolved(t *testing.T) {
	p := proc(1, "invoice", "Invoice", "a1")
	p.Workers = []WorkerUse{{ElementID: "Task_1", Name: "nowhere-rest"}}
	...
	n := nodeByID(t, g, "unresolved:worker:nowhere-rest")
	if n.Kind != KindUnresolved || n.Name != "nowhere-rest" {
		t.Errorf("unresolved worker node = %+v", n)
	}
	if !hasEdge(g, "process:1", n.ID, EdgeUses) {
		t.Errorf("edge to the unconfigured worker missing from %#v", g.Edges)
	}
}

// TestDeriveGraphKeepsTheEdgeToARestrictedWorker applies §3's rule to the second
// resource kind that has a sharing scope of its own (ADR-0205). Same rule, same
// placeholder, and a worker must not merge with a hidden process.
func TestDeriveGraphKeepsTheEdgeToARestrictedWorker(t *testing.T) {
	p := proc(1, "invoice", "Invoice", "a1")
	p.Workers = []WorkerUse{{ElementID: "Task_1", Name: "secret-rest", TargetID: "c1"}}

	g := DeriveGraph(Landscape{
		Applications: []Application{app("a1", "Billing")},
		Processes:    []Process{p},
		Workers:      []Worker{{ID: "c1", Name: "secret-rest", Type: "rest"}},
	}, Options{})

	if got := kindsOf(g); got[KindWorker] != 0 || got[KindRestricted] != 1 {
		t.Fatalf("node kinds = %v, want the worker replaced by a placeholder", got)
	}
	if g.Restricted != 1 {
	...
	if strings.Contains(string(body), "secret-rest") {
		t.Errorf("restricted worker leaks its name in %s", body)
	}
}

// TestRestrictedProcessesAndWorkersDoNotShareAPlaceholder: placeholders are
// keyed per resource, and a hidden process is not a hidden worker. Merging them
// would draw one dependency where there are two, on different things.
func TestRestrictedProcessesAndWorkersDoNotShareAPlaceholder(t *testing.T) {
	hidden := proc(2, "dunning", "Dunning", "a2")
	hidden.CanView = false
...
// already enumerates. A remote decision, evaluated by a worker rather than in this
// engine, is deliberately not here: it arrives as a worker reference instead, which
...
// worker are one dependency, not three. The mesh answers "what does this depend
...
// not an inventory. A configured worker nothing uses is not a landscape edge, and
// putting it on screen would bury the ones that are.
func TestUnreferencedResourcesStayOutOfTheGraph(t *testing.T) {
	g := DeriveGraph(Landscape{
		Applications: []Application{app("a1", "Billing")},
		Processes:    []Process{proc(1, "invoice", "Invoice", "a1")},
		Workers:      []Worker{{ID: "c1", Name: "unused-rest", Type: "rest", CanView: true}},
		Decisions:    []Decision{{ID: "unused", Name: "Unused", CanView: true}},
	}, Options{})

	if got := kindsOf(g); got[KindWorker] != 0 || got[KindDecision] != 0 {
		t.Errorf("node kinds = %v, want no unreferenced worker or decision", got)
	}
}

// TestUnresolvedProcessAndWorkerIdsDoNotCollide pins the namespaced id scheme: a
// process id and a worker name can be the same string, and they are different
// findings about different things.
func TestUnresolvedProcessAndWorkerIdsDoNotCollide(t *testing.T) {
	p := proc(1, "invoice", "Invoice", "a1", Call{ElementID: "Call_1", CalledProcessID: "billing"})
	p.Workers = []WorkerUse{{ElementID: "Task_1", Name: "billing"}}
	...
	if got := kindsOf(g)[KindUnresolved]; got != 2 {
		t.Fatalf("unresolved nodes = %d, want 2 — one process target and one worker", got)
	}
	nodeByID(t, g, "unresolved:process:billing")
	nodeByID(t, g, "unresolved:worker:billing")
}

// TestDeriveGraphHandlesHiddenAndMissingDecisions completes the decision edge for
// the same two cases the worker edge already covers. A decision reached through
