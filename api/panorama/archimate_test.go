...
func TestExportedModelSaysItWasNotDrawn(t *testing.T) {
	...
	for _, want := range []string{
		...
		// The version the tables are at, not a literal: it moves when a row changes
		// meaning, and a document generated before and after is a different document.
		fmt.Sprintf("mapping version %d", NotationMappingVersion),
...
// catalogueLandscape is a catalogue with one of everything the export has to decide
// about: an integral part, an optional one, a precedence the vocabulary has no word
// for, and a product whose provisioning process is deployed here.
func catalogueLandscape() Landscape {
	c := ProductCatalog{
		ID: "cat_1", Name: "Mobile devices", CanView: true,
		Items: []string{"package", "phone", "case", "sim"},
		Edges: []CatalogEdge{
			{From: "package", To: "phone", Kind: "composition"},
			{From: "package", To: "case", Kind: "aggregation"},
			{From: "phone", To: "sim", Kind: "requires"},
		},
	}
	return Landscape{
		Applications: []Application{app("a1", "Workplace")},
		Processes:    []Process{proc(1, "provision-phone", "Provision a phone", "a1")},
		Catalogs:     []ProductCatalog{c},
		Products: []Product{
			{ID: "package", Name: "Phone package", HomeCatalog: "cat_1", CanView: true},
			{ID: "phone", Name: "Apple iPhone", HomeCatalog: "cat_1", CanView: true,
				ProvisionProcess: "provision-phone"},
			{ID: "case", Name: "Protective case", HomeCatalog: "cat_1", CanView: true},
			{ID: "sim", Name: "SIM card", HomeCatalog: "cat_1", CanView: true},
		},
	}
}

// TestExportedCatalogueUsesArchiMatesOwnWords.
//
// The catalogue is the first thing on this landscape ArchiMate can name exactly: an
...
func TestExportedCatalogueUsesArchiMatesOwnWords(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(xml, want) {
			t.Errorf("the document carries no %s", want)
		}
	}
	// And the product still reaches the process that provisions it, through the same
	// Serving the rest of the landscape uses.
	if !strings.Contains(xml, `xsi:type="Serving"`) {
		t.Errorf("the product's provisioning process is not related to it")
	}
}

// TestExportedPrecedenceIsAbsentAndSaysWhy.
//
// ArchiMate has no relationship meaning "this cannot be provisioned before that"
// between two products, so the line is on the picture and not in the file. What must
...
func TestExportedPrecedenceIsAbsentAndSaysWhy(t *testing.T) {
	...
	if !strings.Contains(xml, "have no ArchiMate relationship to be written as") {
		t.Errorf("the document does not say a relationship was left out for want of a word for it")
	}
	if strings.Contains(xml, "had an end that is not in this model") {
		t.Errorf("a relationship both of whose ends are exported was reported as having a missing end")
	}
	for _, loss := range []string{"Precedence between two products is drawn and not exported"} {
		if !strings.Contains(xml, loss) {
			t.Errorf("the loss list does not say %q", loss)
		}
	}
}
