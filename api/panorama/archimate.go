...
func ExportArchiMate(g Graph, opts ArchiMateExport) []byte {
	...
	// Two ways a line on the starmap fails to become a relationship in the document,
	// counted apart because they say different things to a reader. One is about this
	// reader's access; the other is about this vocabulary, and no amount of access
	// would fix it.
	dropped, unmapped := 0, 0
	for i, e := range g.Edges {
		rel, known := archiRelations[e.Kind]
		from, haveFrom := ids[e.From]
		to, haveTo := ids[e.To]
		if !known {
			// The relationship itself has no ArchiMate type. Both ends may well be in
			// the document — precedence between two products is exactly that — and
			// reporting it as a missing end would send the reader looking for an
			// element that is right there.
			unmapped++
			continue
		}
		if !haveFrom || !haveTo {
	...
	out.WriteString("  <documentation xml:lang=\"en\">" +
		escapeXML(provenance(opts, notation, len(exported), restricted, dropped, unmapped)) + "</documentation>\n")
	if elements.Len() > 0 {
...
func provenance(opts ArchiMateExport, notation Notation, elements, restricted, dropped, unmapped int) string {
	...
	if unmapped > 0 {
		b.WriteString(fmt.Sprintf("%d relationship(s) are on the starmap and have no ArchiMate "+
			"relationship to be written as; both of their ends are here and the line between "+
			"them is not. The list below says which. ", unmapped))
	}
