...
// Who owns a worker, and who may touch it (ADR-0205, measure M11).
//
// Before this, every authenticated account could list every worker with its
...
// resolving a worker regardless of who started it. Execution is not authoring
// (ADR-0071), and a sharing rule that reached the runtime would break every model
// the moment its author went on holiday.

// connectorPayload is the smallest valid create body: a clio worker needs a name
...
// createConnector posts a worker and returns its id.
func createConnector(t *testing.T, c *http.Client, base, name string) string {
	...
	if resp.StatusCode != http.StatusOK {
		...
		t.Fatalf("create worker = %d: %s", resp.StatusCode, data)
	}
...
func listConnectors(t *testing.T, c *http.Client, base string) map[string]map[string]any {
	...
	if resp.StatusCode != http.StatusOK {
		data, _ := io.ReadAll(resp.Body)
		t.Fatalf("list workers = %d: %s", resp.StatusCode, data)
	}
...
// shareConnector grants a principal a role on a worker.
func shareConnector(t *testing.T, c *http.Client, base, connID, principalID, refType, role string, want int) {
	...
	if got := statusOf(t, c, http.MethodPut, base+"/api/v1/connectors/"+connID+"/members/"+principalID, body); got != want {
		t.Fatalf("share worker = %d, want %d", got, want)
	}
}

// TestAConnectorBelongsToWhoeverMadeIt is the whole measure in one test: the five
// things an ordinary account could do to a stranger's worker, and cannot now.
func TestAConnectorBelongsToWhoeverMadeIt(t *testing.T) {
	...
	t.Run("its owner keeps it", func(t *testing.T) {
		...
		if seen[id] == nil {
			t.Fatal("the owner cannot see her own worker")
		}
	...
	t.Run("a stranger sees no configuration", func(t *testing.T) {
		...
		// service task against a worker that exists (see the catalog test below).
		// What must not travel is the configuration.
		if c := seen[id]; c != nil {
			for _, field := range []string{"endpoint", "sender", "credentialsRef", "ownerId", "members"} {
				if _, ok := c[field]; ok {
					t.Errorf("a stranger sees %q on somebody else's worker: %v", field, c)
				}
			}
		}
	})
	...
	t.Run("a stranger cannot change it", func(t *testing.T) {
		if got := statusOf(t, bert, http.MethodPatch, ts.URL+"/api/v1/connectors/"+id, `{"enabled":false}`); got == http.StatusOK {
			t.Error("a stranger edited somebody else's worker")
		}
	})
	...
	t.Run("a stranger cannot delete it", func(t *testing.T) {
		if got := statusOf(t, bert, http.MethodDelete, ts.URL+"/api/v1/connectors/"+id+"?force=true", ""); got == http.StatusNoContent {
			t.Error("a stranger deleted somebody else's worker — the reported hole")
		}
		if listConnectors(t, anna, ts.URL)[id] == nil {
			t.Error("the worker is gone after a stranger's delete attempt")
		}
	})
	...
	t.Run("a stranger cannot subscribe on it", func(t *testing.T) {
		...
		if got := statusOf(t, bert, http.MethodPost, ts.URL+"/api/v1/connectors/"+id+"/inbound-subscriptions", body); got == http.StatusOK {
			t.Error("a stranger pointed somebody else's worker at a message name of their choosing")
		}
	})
	...
	t.Run("an administrator still sees everything", func(t *testing.T) {
		...
		if seen[id] == nil || seen[id]["endpoint"] == nil {
			t.Error("an administrator cannot see a worker — that is what admin means here")
		}
	})
}
...
//
// The modeler fills its worker picker from GET /api/v1/connectors and needs a
...
func TestEveryoneCanStillAuthorAgainstAConnector(t *testing.T) {
	...
	if seen == nil {
		t.Fatal("a worker is invisible to a modeller, who then cannot author against it")
	}
...
func TestSharingAConnectorFollowsTheRoles(t *testing.T) {
	...
	t.Run("viewer reads and does not write", func(t *testing.T) {
		...
		if got := statusOf(t, bert, http.MethodPatch, ts.URL+"/api/v1/connectors/"+id, `{"enabled":false}`); got == http.StatusOK {
			t.Error("a viewer changed the worker")
		}
	})
	...
	t.Run("editor writes and does not share or delete", func(t *testing.T) {
		...
		if got := statusOf(t, bert, http.MethodPatch, ts.URL+"/api/v1/connectors/"+id, `{"enabled":true}`); got != http.StatusOK {
			t.Errorf("an editor cannot change the worker: %d", got)
		}
		...
		if got := statusOf(t, bert, http.MethodDelete, ts.URL+"/api/v1/connectors/"+id+"?force=true", ""); got == http.StatusNoContent {
			t.Error("an editor deleted the worker — deleting is the owner's")
		}
	})
...
func TestSharingAConnectorWithAGroup(t *testing.T) {
	...
	bert := signInAs(t, ts.URL, "bert", "a-password-that-is-long")
	if seen := listConnectors(t, bert, ts.URL)[id]; seen == nil || seen["endpoint"] == nil {
		t.Fatalf("a group member cannot see the worker: %v", seen)
	}
...
func TestAConnectorFromBeforeOwnershipIsAdminOnly(t *testing.T) {
	...
	// An admin-created worker, stripped of its owner on disk: exactly the shape a
	...
	if seen := listConnectors(t, anna, ts.URL)[id]; seen != nil && seen["endpoint"] != nil {
		t.Error("an ownerless worker still shows its configuration to everybody")
	}
	if seen := listConnectors(t, anna, ts.URL)[id]; seen == nil {
		t.Error("an ownerless worker vanished from the catalog, which breaks authoring against it")
	}
	if got := statusOf(t, anna, http.MethodPatch, ts.URL+"/api/v1/connectors/"+id, `{"enabled":false}`); got == http.StatusOK {
		t.Error("an ordinary account still edits an ownerless worker")
	}

	admin = signedInClient(t, ts.URL)
	if got := statusOf(t, admin, http.MethodPatch, ts.URL+"/api/v1/connectors/"+id, `{"enabled":false}`); got != http.StatusOK {
		t.Errorf("an administrator cannot manage an ownerless worker: %d", got)
	}
}
...
func TestAnOpenServerSharesEverything(t *testing.T) {
	...
	if seen := listConnectors(t, c, ts.URL)[id]; seen == nil || seen["endpoint"] == nil {
		t.Fatalf("an open server hid a worker: %v", seen)
	}
...
// TestBorrowingAnotherConnectorsCredentialIsRefused closes what the worker check
...
//
// Locking the worker record while anyone may still borrow its credential would
// be theatre. So the rule is exact: you may name a credential reference only if a
// worker you may edit already uses it.
func TestBorrowingAnotherConnectorsCredentialIsRefused(t *testing.T) {
	...
	// The house mail worker, owned by the administrator. Deliberately not the
	...
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("create mail worker = %d", resp.StatusCode)
	}
...
// stripConnectorOwner rewrites a stored worker into the shape it had before
...
func stripConnectorOwner(t *testing.T, dir, id string) {
	...
	if rec == nil {
		t.Fatalf("no stored worker with id %s", id)
	}
...
// TestSealingAConnectorAndOpeningItAgain: an owner can shut a shared worker
...
// owner who wanted to close a worker for an afternoon would have to rebuild the
...
func TestSealingAConnectorAndOpeningItAgain(t *testing.T) {
	...
	if listConnectors(t, bert, ts.URL)[id]["endpoint"] == nil {
		t.Fatal("the member cannot see the worker they were just shared")
	}
	...
	if listConnectors(t, bert, ts.URL)[id]["endpoint"] != nil {
		t.Error("sealing the worker left its member with access")
	}
	...
	t.Run("only the owner decides", func(t *testing.T) {
		...
		if got := statusOf(t, bert, http.MethodPut, visibility, `{"visibility":"private"}`); got != http.StatusForbidden {
			t.Errorf("= %d, want 403: an editor sealed somebody else's worker", got)
		}
	})
...
// TestHandingAConnectorOn: a worker outlives the person who made it.
//
// It exists because an ownerless worker is administrators-only. Without a
// transfer, one person leaving would make an administrator the owner of every
// worker they ever configured — and the fix would be editing JSON on disk.
func TestHandingAConnectorOn(t *testing.T) {
	...
	t.Run("a stranger cannot hand on what is not theirs", func(t *testing.T) {
		if got := statusOf(t, bert, http.MethodPut, owner+bertID, ""); got != http.StatusForbidden {
			t.Errorf("= %d, want 403: an editor handed a worker to themselves", got)
		}
	})
	...
	if seen := listConnectors(t, bert, ts.URL)[id]; seen == nil || seen["role"] != "owner" {
		t.Errorf("the new owner does not hold the worker: %v", seen)
	}
	...
	if members, _ := seen["members"].([]any); len(members) != 0 {
		t.Errorf("the new owner is still listed as a member of their own worker: %v", members)
	}
	// Anna handed it over, so she has it no longer.
	if seen := listConnectors(t, anna, ts.URL)[id]; seen != nil && seen["endpoint"] != nil {
		t.Error("the previous owner kept access after handing the worker on")
	}
}
...
func TestSharingRefusesWhatItCannotMean(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// their own worker list them as its editor.
		{"the owner as their own member", annaID, `{"role":"editor"}`, http.StatusBadRequest},
		{"a worker that is not theirs", bertID, `{"role":"viewer"}`, http.StatusOK},
	} {
	...
	t.Run("a worker that does not exist is not there", func(t *testing.T) {
...
// who owns the vault able to check a worker. Otherwise an administrator
// diagnosing somebody else's mail worker would be refused by a measure meant to
// stop a stranger.
func TestAnAdministratorMayUseAnyCredential(t *testing.T) {
	...
	if got := statusOf(t, admin, http.MethodPost, ts.URL+"/api/v1/connectors/test", probe); got != http.StatusOK {
		t.Errorf("= %d, want 200: an administrator was refused a worker check", got)
	}
}
...
// endpoint that reported "no worker with that id" for an unreadable store would
// look exactly like a worker somebody had deleted.
func TestConnectorSharingSurvivesABrokenDataDirectory(t *testing.T) {
	...
	// The worker check reads the same store to decide whether a credential may be
	...
	if got := statusOf(t, carla, http.MethodPost, ts.URL+"/api/v1/connectors/test", probe); got != http.StatusInternalServerError {
		t.Errorf("worker check = %d, want 500", got)
	}
}
