			...
			if ei, ok, err := s.store.GetElementInstance(jv.ElementInstanceKey); err == nil && ok {
				...
				if d, dok := s.deployments[ei.ProcessDefKey]; dok {
					...
					if n := cp.Node(ei.ElementId); n.Type == compiler.TypeUserTask {
						...
						// rewrite it, ADR-0038); candidate groups stay the
...
// reassigns it (last-writer-wins, ADR-0038). 400 on a missing/empty assignee,
...
// again. 404 if the job doesn't exist or is already completed (ADR-0038).
