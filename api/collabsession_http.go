...
// decision (ADR-0321). Rather than a second copy of seven handlers, they take a
// subject: the key the registry files the session under, and how a request is
// authorized against the artifact behind it. This is affordable precisely because
// a session holds no durable state — there are no records to migrate and no
// stored field to rename.
type sessionSubject struct {
	// key namespaces the registry, so a diagram draft and a decision draft that
	// happen to share an id are two sessions rather than one silently shared.
	key func(id string) string
	// access authorizes joining and reports whether the caller may edit. Status 0
	// means granted; anything else is written to the response as-is.
	access func(r *http.Request, id string) (canEdit bool, status int, msg string)
	// absent is the 404 a stale participant gets, naming the artifact in the words
	// the caller would use for it.
	absent string
}

// bpmnDraftSession is the subject ADR-0140 shipped: a BPMN draft, keyed by its
// process id, authorized against its project.
func (s *Server) bpmnDraftSession() sessionSubject {
	return sessionSubject{
		key:    func(id string) string { return id },
		access: s.draftSessionAccess,
		absent: "no such participant in this draft's session",
	}
}

// dmnDraftSession is the same for a decision draft. The key is prefixed so the
// two namespaces cannot collide in the registry.
func (s *Server) dmnDraftSession() sessionSubject {
	return sessionSubject{
		key:    func(id string) string { return "dmn:" + id },
		access: s.dmnDraftSessionAccess,
		absent: "no such participant in this decision's session",
	}
}
...
// with no access at all gets 404, so a scoped decision's existence never leaks.
func (s *Server) dmnDraftSessionAccess(r *http.Request, draftID string) (canEdit bool, status int, msg string) {
	var (
		rec     dmnDraft
		draftOK bool
		proj    project
	...
	s.do(func() {
		if rec, draftOK, readErr = s.dmnDrafts.Get(draftID); readErr != nil || !draftOK {
			return
		}
		if rec.ProjectID != "" {
			proj, projOK, readErr = s.projects.Get(rec.ProjectID)
	...
	switch {
	case readErr != nil:
		return false, http.StatusInternalServerError, "read decision draft: " + readErr.Error()
	case !draftOK:
		return false, http.StatusNotFound, "no decision draft with that id"
	}
	if rec.ProjectID == "" || !projOK {
		return true, 0, ""
	}
	rank := scopeRank(proj.effectiveRole(httpapi.PrincipalFrom(r.Context()), s.authEnabled))
	if rank == 0 {
		return false, http.StatusNotFound, "no decision draft with that id" // hide existence
	}
	return rank >= scopeRank(ScopeRoleEditor), 0, ""
}
...
func (s *Server) requireSessionEditor(w http.ResponseWriter, subject sessionSubject, key, participantID string) bool {
	canEdit, ok := s.collab.CanEdit(key, participantID)
	if !ok {
		httpapi.Error(w, http.StatusNotFound, subject.absent)
		return false
	}
	if !canEdit {
...
func (s *Server) handleDraftSession(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		...
		canEdit, status, msg := subject.access(r, r.PathValue("id"))
		if status != 0 {
...
func (s *Server) handleDraftSessionJoin(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
...
func (s *Server) handleDraftSessionPoll(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		var body struct {
...
func (s *Server) handleDraftSessionLeave(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		var body struct {
			ParticipantID string `json:"participantId"`
...
func (s *Server) handleDraftSessionPresence(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		var body struct {
			ParticipantID string `json:"participantId"`
		...
		if !s.requireSessionEditor(w, subject, id, body.ParticipantID) {
			return
		}
...
func (s *Server) handleDraftSessionLock(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		var body struct {
			ParticipantID string `json:"participantId"`
...
func (s *Server) handleDraftSessionChange(subject sessionSubject) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id := subject.key(r.PathValue("id"))
		var body struct {
			ParticipantID string `json:"participantId"`
