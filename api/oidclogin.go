...
// created account gets (ADR-0209). Mapping claims onto roles is a later step and a
// deliberate one, because from the day it ships whoever administers the provider's
...
func (s *Server) handleOIDCCallback(w http.ResponseWriter, r *http.Request) {
	...
	u, err := s.oidcAccount(claims, now.Unix())
	if err != nil {
...
func (s *Server) oidcAccount(claims oidcClaims, now int64) (User, error) {
	var (
		out User
		err error
	)
	s.do(func() {
		...
		if found {
			...
			if changed := applyOIDCProfile(&out, claims); !changed {
				return
			}
			out.UpdatedAt = now
			err = s.users.Save(out)
			return
		}
		var id string
