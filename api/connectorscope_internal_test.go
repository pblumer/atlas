...
// The rule behind connector ownership, tested where it is decided (ADR-0205), and
...
func TestConnectorRoleIsTheProjectRuleWithOneDeparture(t *testing.T) {
	...
	// Members on a private connector are inert rather than deleted: sealing a
	// connector and opening it again must not cost the owner the list.
	...
	for _, tc := range []struct {
		...
		{"a shared connector grants its member", shared, bert, true, ScopeRoleViewer},
		...
		{"a legacy connector is nobody's", legacy, anna, true, ""},
		{"a legacy connector is still an administrator's", legacy, admin, true, ScopeRoleOwner},
		{"a legacy connector is open when auth is off", legacy, nil, false, ScopeRoleOwner},
	} {
...
// connector is not there, and one with too little is told they may not. The
// difference matters — 403 on a connector somebody cannot see would confirm that it
// exists, which is exactly what a private connector must not do.
func TestCheckConnectorRoleHidesRatherThanForbids(t *testing.T) {
	...
	if code, _ := s.checkConnectorRole(req, c, ScopeRoleViewer); code != 404 {
		t.Errorf("a caller with no access got %d, want 404 — 403 would confirm the connector exists", code)
	}
...
//
// Execution is not authoring (ADR-0071). A deployed process resolves a connector by
// name, on the run loop, with no request and no principal anywhere near it — so a
// connector owned by one person and private to them must still be *in the
...
func TestTheRuntimeResolvesAConnectorNobodyIsSignedInFor(t *testing.T) {
	...
	// Anna's private mail connector, on the preview provider so building its client
	// dials nothing.
	rec := connector{
	...
	if problem != "" {
		t.Errorf("the runtime cannot use a private connector: %q — ownership reached execution, "+
