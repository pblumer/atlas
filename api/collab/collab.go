// Package collab implements the first slice of ADR-0140: live collaborative
// modeling sessions. A collaboration session is an ephemeral, design-time coordination
...
// six invariants.
//
// Concurrency model (ADR-0140, first cut): per-element soft locks. An editor
...
// Collaboration event types fanned out on a session's stream. Mirrors ADR-0140:
...
// one (reconnect/replay is an ADR-0140 follow-up, not needed for correctness).
const (
	EventSync           = "sync"
...
// bufferSize bounds a participant's outgoing event buffer. A consumer that
...
// Event is one frame delivered to a participant's SSE stream. Data is the
// already-marshaled JSON payload; Seq is a per-session monotonic counter carried
// as the SSE id so a future reconnect can resume (ADR-0140 follow-up).
type Event struct {
	Type string
	Data []byte
	Seq  uint64
...
// Participant is one connected editor — a person in the Modeler or an AI
...
// Events is the participant's outgoing frame stream, consumed by the SSE handler
// that owns this participant. It is closed when the participant is reaped or the
// session torn down, which is how that handler learns to end the response.
func (p *Participant) Events() <-chan Event { return p.ch }

// CanEdit reports whether a participant may mutate the session (lock/change/
// presence), snapshotted from its project role at join time (ADR-0140/0071). ok
...
// session is the live coordination state for one draft: its participants,
// its held element locks, and a monotonic sequence counter.
type session struct {
	participants map[string]*Participant
	locks        map[string]Lock // elementID → lock
	seq          uint64
}
...
func (sess *session) broadcast(typ string, data []byte) {
...
func (sess *session) broadcastExcept(typ string, data []byte, except string) {
	sess.seq++
	ev := Event{Type: typ, Data: data, Seq: sess.seq}
...
// ParticipantTTL is how long any participant may go without a sign of life
...
// forever. ReapInterval is how often the reaper sweeps.
const (
	ParticipantTTL = 90 * time.Second
	ReapInterval   = 30 * time.Second
)

// KeepaliveInterval is how often an idle SSE session stream writes a keepalive
...
// Registry holds every live draft session in memory. It is mutex-guarded
// because concurrent HTTP handlers (SSE streams and POSTs) reach it directly; it
// is not engine state and never persists (ADR-0140).
...
// randomHex returns n cryptographically random bytes as hex. Participant ids must
// be unguessable — holding one is what lets a request act as that participant — so
// they are minted the same way session tokens are, not from a counter.
func randomHex(n int) (string, error) {
	b := make([]byte, n)
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("collab: random: %w", err)
	}
	return hex.EncodeToString(b), nil
}

// NewRegistry builds an empty registry whose participant ids are random
...
func (sess *session) rosterLocked() []Presence {
	out := make([]Presence, 0, len(sess.participants))
	for _, p := range sess.participants {
		out = append(out, Presence{ID: p.ID, UserID: p.UserID, Name: p.Name, Selection: p.selection})
	}
	return out
}

// lockListLocked builds the current lock set. Caller holds the mutex.
func (sess *session) lockListLocked() []Lock {
	out := make([]Lock, 0, len(sess.locks))
...
// handler uses to pass the draft's project role (ADR-0140/0071).
...
func (reg *Registry) joinDetached(draftID, userID, name string) (*Participant, []byte) {
...
// ADR-0140 M2), carrying its edit capability. It returns the participant and its
...
func (reg *Registry) joinLocked(draftID, userID, name string, detached, canEdit bool) (*Participant, []byte) {
	sess := reg.sessions[draftID]
	if sess == nil {
		sess = &session{participants: map[string]*Participant{}, locks: map[string]Lock{}}
	...
	p := &Participant{
		...
		lastSeen: reg.now(), ch: make(chan Event, bufferSize),
	}
...
func (reg *Registry) removeParticipantLocked(sess *session, participantID string) (released bool) {
...
func (reg *Registry) AcquireLock(draftID, participantID, elementID string) (granted, ok bool) {
	...
	sess.locks[elementID] = Lock{ElementID: elementID, HolderID: participantID, HolderName: p.Name}
	reg.broadcastLocksLocked(sess)
	return true, true
}
...
// live stream (an AI agent over MCP, ADR-0140 M2).
...
func (reg *Registry) broadcastPresenceLocked(sess *session) {
...
func (reg *Registry) broadcastPresenceExceptLocked(sess *session, except string) {
	data, _ := json.Marshal(struct {
		Participants []Presence `json:"participants"`
...
// mutex.
func (reg *Registry) broadcastLocksLocked(sess *session) {
	data, _ := json.Marshal(struct {
		Locks []Lock `json:"locks"`
