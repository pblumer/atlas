...
func TestSearchByUndeclaredVariableStillWalks(t *testing.T) {
	...
	code, body := doReq(t, ts, http.MethodGet, fmt.Sprintf("/api/v1/instances/search?process=%d&q=%s", def, url.QueryEscape("nachname=Test")), "", "")
	if code != http.StatusOK {
	...
	if len(rows) != 1 {
		t.Errorf("nachname=Test matched %d rows, want 1 (substring, as before)", len(rows))
	}
}
