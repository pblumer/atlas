...
// (ADR-0090) — mirroring the mail/rest/DMN workers' ProcessLookup.
...
// when its source/result variables are unset (ADR-0090).
const (
...
//
//   - A first-class CSV-to-JSON connector task (ADR-0090): the source variable,
...
func csvImportHandler(store csvVarStore, lookup csvProcessLookup) job.OutputHandler {
	return func(j job.Job) ([]model.VariableValue, error) {
		...
		// Prefer the compiled connector detail (ADR-0090) when the task is an
...
// csvRowsFromConnector runs a CSV-to-JSON connector task (ADR-0090): it reads the raw
