...
// script task marked with a BPMN standard loop (ADR-0130): it recomputes n from
...
// enclosing scope, so the rest of the process sees it (ADR-0130).
...
// taking its outgoing flow (ADR-0130).
...
// like any activity, rather than spinning forever (ADR-0130).
...
// worker's output feeds the condition that decides on the next round (ADR-0130).
...
// (ADR-0018 recovery contract, ADR-0130).
...
// (ADR-0130 — inherited from ADR-0077, asserted here because a loop that cannot be
