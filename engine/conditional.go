...
// over the scope chain starting at scopeKey (ADR-0134) — the same eval a gateway condition
...
// variable in the instance changed (ADR-0134). It is a transient command-only handler — never
...
// conditional catch/boundary whose condition now holds (ADR-0134). A conditional evaluates over
...
// armed conditional catch or a conditional boundary, or nil otherwise (ADR-0134).
...
// conditionalCatchBehavior: a conditional intermediate catch event (ADR-0134). It arms inert —
