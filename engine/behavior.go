...
func handleIncidentResolved(c *ProcessingContext) {
	...
	if inc.JobKey == 0 {
		// A timer incident carries no job (ADR-0062): re-arm the parked catch/boundary
...
// whose FEEL schedule failed and raised an incident (ADR-0062). Re-evaluated
...
// message (ADR-0062). A non-FEEL schedule always resolves.
...
// resolve it (ADR-0062). Either way the element stays Activated; only a created
...
func (timerCatchEventBehavior) OnActivated(c *ProcessingContext, key uint64, ei *model.ElementInstanceValue) {
	...
	// FEEL schedule — raises an incident and parks (ADR-0062).
...
func (boundaryEventBehavior) OnActivated(c *ProcessingContext, key uint64, ei *model.ElementInstanceValue) {
	...
	case compiler.BoundaryTimer:
		...
		// incident and parks instead of firing immediately (ADR-0062).
