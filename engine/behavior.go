...
func ioResultScope(cp *compiler.CompiledProcess, elementKey uint64, ei *model.ElementInstanceValue) uint64 {
	...
	// promotes them when the loop ends (ADR-0131). Otherwise the local scope when the
...
func seedMultiInstance(c *ProcessingContext, bodyKey uint64, ei *model.ElementInstanceValue) {
	...
	if d.Standard {
		// A standard loop's iteration set is a condition, not a collection (ADR-0131):
...
// (ADR-0131): its cap is not reached and its condition still holds. done is the number
...
// multi-instance one (ADR-0131). The two share the compiled loop table and the body/
...
func finishMultiInstanceIteration(c *ProcessingContext, key uint64, ei *model.ElementInstanceValue) {
	...
	// The completion condition — for a standard loop, the loop condition (ADR-0131) —
...
func promoteMultiInstanceOutput(c *ProcessingContext, bodyKey uint64, ei *model.ElementInstanceValue) {
	...
	if d.Standard {
		...
		// (ADR-0131). Promoting all of it to the enclosing scope makes a looping activity
