...
func TestHandlerFailsWhenTheStoreDoes(t *testing.T) {
	...
	if _, err := sharepoint.Handler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); !errors.Is(err, boom) {
	...
	if vars, err := sharepoint.Handler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); err != nil || vars != nil {
