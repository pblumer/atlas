...
// job worker (every worker) or is called by one (the DMN model resolver).
...
// this package exists to prevent: a worker using http.DefaultClient, whose
// zero Timeout means "wait forever". Because workers run ON the
...
//
// A new worker must therefore build its client from nettimeout (or set its
