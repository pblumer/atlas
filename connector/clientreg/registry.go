// Package clientreg is the client registry every Worker Type shares: a
// name → client map that a worker resolves a model's worker reference through,
// plus the part that is not a map — *why* a worker an operator did configure is
// missing from it.
//
// A worker that cannot be built (an endpoint naming no host, a credential bundle
...
// worker registered as X" — which reads as *you never configured it* and sends an
// operator looking for a worker that is sitting right there, disabled or broken.
...
// Registry resolves a worker name to the client of its kind, and remembers the
// configured workers of that kind it could not build.
...
// Register binds a worker name to its client. Registering the same name again
...
// Problem returns why a *configured* worker of this kind is not usable, and
...
// Replace swaps the whole set of registered workers at once, so a server can
...
// ReplaceWith swaps the registered workers *and* the reasons the configured ones
...
// Problems returns every recorded reason, keyed by worker name. The caller must
// not mutate the result. It backs the operator view that lists a configured
// worker's health beside it.
func (r *Registry[T]) Problems() map[string]string { return r.problems }

// Unresolved is the error a worker returns when a model's worker reference does
...
// Worker Type, as every worker's errors do.
func (r *Registry[T]) Unresolved(kind, name string) error {
	if why, ok := r.Problem(name); ok {
		return fmt.Errorf("%s: worker %q is configured but not usable: %s", kind, name, why)
	}
	return fmt.Errorf("%s: no worker registered as %q", kind, name)
}
