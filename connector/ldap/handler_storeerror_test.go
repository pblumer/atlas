...
func TestHandlerFailsWhenTheStoreDoes(t *testing.T) {
	...
	if _, err := ldap.Handler(store, lookup, nil, nil)(job.Job{ElementInstanceKey: 1}); !errors.Is(err, boom) {
	...
	if vars, err := ldap.Handler(store, lookup, nil, nil)(job.Job{ElementInstanceKey: 1}); err != nil || vars != nil {
