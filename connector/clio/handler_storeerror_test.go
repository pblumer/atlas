...
func TestHandlerFailsWhenTheStoreDoes(t *testing.T) {
	...
	// clio's write handler returns no variables, so it is a plain job.Handler; the
	// read handler is the OutputHandler. Both guard the same way.
	store := state.Reader(unreadableStore{err: boom})
	if err := clio.Handler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); !errors.Is(err, boom) {
		t.Errorf("write: error = %v, want the store's own failure rather than a silent no-op", err)
	}
	if _, err := clio.ReadHandler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); !errors.Is(err, boom) {
		t.Errorf("read: error = %v, want the store's own failure rather than a silent no-op", err)
	}
	...
	if err := clio.Handler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); err != nil {
		t.Errorf("gone element instance: err=%v, want none", err)
	}
	if vars, err := clio.ReadHandler(store, lookup, nil)(job.Job{ElementInstanceKey: 1}); err != nil || vars != nil {
