...
# alone takes over twenty minutes under -race, so the default fails
...
api/        HTTP API, web UI, command submission and queries
  ...
  layout/     BPMN diagram auto-layout, and transplanting an adjusted diagram onto a
              deployed model (ADR-0124/0127, ADR-0251)
...
the failure it is usually reported with, the anchor of its handbook card in
...
6. **If you changed an architectural decision**, write a new ADR instead of silently diverging — and **do not give it a number**. Copy [`docs/adr/template.md`](docs/adr/template.md) to `docs/adr/draft-<slug>.md`, keep its `# ADR-DRAFT: Title` heading, and add **no** row to [`docs/adr/README.md`](docs/adr/README.md). Set **both** state lines: `Status` says whether the decision holds, `Implementation` says whether it is built, and `go test ./docs/adr` refuses `Implementation: Landed` under any status but `Accepted` — a record that arrives with its code is `Accepted` / `Landed` (see [`docs/adr/README.md`](docs/adr/README.md#the-two-states-of-a-record)). If the record is right only while something you could not settle stays unknown, say so in its front matter with an `Open question` and a `Question checked` month — `go test ./docs/adr` then insists somebody looks at that question once a year, so the gap cannot quietly become settled truth (see [`docs/adr/README.md`](docs/adr/README.md#when-the-record-rests-on-a-question-you-could-not-answer)). Name what you looked at outside Atlas in a `Prior art` line — the standards, specifications and existing implementations you examined, or `none` and why none applies — because a record that names none cannot be told apart from one that examined prior art and refused it, and `go test ./docs/adr` fails on a record that omits the line or answers `none` without a reason (see [`docs/adr/README.md`](docs/adr/README.md#the-prior-art-line); ADR-0387 is the case that bought the rule). The number is assigned when the record lands on main, by a workflow that runs `make adr-number` there; picking one on a branch is how two records end up sharing it, and how a record gets renumbered on every merge. Cite the record as `ADR-draft-<slug>` (or link `draft-<slug>.md`) from code and docs — the numbering rewrites those citations along with the file name and adds the index row. `go test ./docs/adr` guards all of it and runs in the normal test sweep. See [`docs/adr/README.md`](docs/adr/README.md#writing-a-record) for the whole flow.
...
- **A new compiler rule must not be able to stop the server from starting.** Every deployed definition is recompiled at boot, so a rule you add today is applied to models deployed long before it existed. That rule decides whether a model may be *deployed*, never whether it may be *loaded* (ADR-0177, ADR-0393). The mechanism is not automatic and it is not the stage: `parseNamed`'s reload escape hatch only recognises a `*ValidationError` carrying a compiled process, so a refusal raised anywhere earlier — in the build stage, where `compileFEEL` lives — arrives as a plain error and is fatal at boot. That is not theoretical: refusing a FEEL call that can only ever be null (ADR-0388) was implemented at that door, and one stored probe model took a production server into a crash loop with every other definition and fifty thousand instances behind it. If your rule refuses something the engine can still *run*, make sure the reload path can keep it and report it instead — a compile that produces nothing runnable is the one case that stays fatal (ADR-0019).
...
| Continue an instance in a new one when its tokens cannot be rebound | `POST /api/v1/instances/{key}/migrate/fork` — [ADR-0389](docs/adr/0389-forked-instance-migration.md) |
